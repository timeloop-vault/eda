...
start() ->
  ...
  {ok, GatewayConnPid} = gun:open(?DISCORDGATEWAYURL, ?DISCORDGATEWAYPORT, #{protocols=>[http],
                                                                      trace=>false,
                                                                      ws_opts=>#{compress=>true}}),
  {ok, RestApiConnPid} = gun:open(?DISCORDRESTAPIURL, ?DISCORDRESTAPIPORT, #{protocols=>[http],
                                                                             trace=>false}),
...
%%generate_snowflake() ->
%%  <<Snowflake:64/integer>> = snowflake:serialize(snowflake:new()),
%%  Snowflake.
